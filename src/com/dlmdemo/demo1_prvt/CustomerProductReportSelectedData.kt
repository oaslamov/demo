...
class CustomerProductReportSelectedData: SelectedData<Customer_Product_Report> {
     ...
     * Нам нужна сумма по товару и клиенту. Ее можно считать двумя способами:
     ...
     *
     * Оптимизируем.
     * 1) мы знаем, что товарных позиций у нас 10 тыс. штук, а значит мы можем сохранить их в памяти, даже если клиент за период покупал из все.
     * (объект с id товара, числовое поле и поле типа BigDecimal - это порядка 8+8+65 байт плюс накладные 20 байт плюс накладные на массив порядка 8 байт умножить на 10000 = порядка 100 килобайт памяти)
     * Это значит, что все товары одного заказа мы прочитаем за один раз (это один или соседние блоки в БД) - это эффективно.
     * 2) у нас интервал по датам и индекс на дату в таблице Shipping_Order. Значит эффективнее будет начинать выборку с нее, а не с клиента. Тогда мы бы прочитали
     * каждый заказ тоже только один раз, да еще ограничили количество строк по индексу.
     * Однако, клиент мог делать несколько заказов за период, и нам нужно их найти. Хранить в памяти всех клиентов и все товары для них нельзя: памяти не хватит, кроме
     * того, если у нас запросят только одного клиента и потом перестанут просить строки, окажется, что мы зря обрабатывали всех (это неэффективно).
     *
     * Для хранения строки по товару в памяти это оправдано, см. выше про хранение содержимого заказа рядом на диске.
     *
     * Чтобы обеспечить:
     * 2.1) чтение таблицы заказов один раз
     * 2.2) не читать все данные заранее и не хранить суммы по всем клиентам в памяти
     * мы схитрим (Алле привет). Если сортировать выборку из Shipping_Order по полю Customer, то все заказы для одного клиента мы получим подряд.
     * То есть мы можем читать заказы пока не изменится поле customer, что будет означать, что заказов для этого клиента на период больше нет.
     ...
     * Нам понадобиться хранить в памяти:
     * 1) данные для строки отчета по каждому товару для текущего клиента (не забываем, почему в данном случае это можно - см. выше)
     * 2) последний считанный из базы данных Shipping_Order, по которому мы поняли, что клиент сменился - его надо будет обработать для следующего клиента, а не для текущего
     *
     * Читать будем так:
     * 1) возвращаем строки отчета из памяти, пока они есть
     ...
     * 4) все данные для текущего клиента готовы, переходим в начало чтения
     *
     ...
     * 2) просуммировать данные по товару в памяти (HashMap по id товара)
     */
